...
| `/design-tokens` (44px touch-target floor) | The home dashboard's arrange-mode tile affordances (`src/renderer/src/modules/home/dashboard/DashboardTile.tsx`) — move grip, resize grip and "return to catalog" — and the dashboard's own arrange toggle (`ArrangeToggle.tsx`, a `size="sm"` icon-only button docked into the arrange bar's slot) use `size="sm"` (28px) IconButtons. | Same reason as above — a desktop, mouse-and-keyboard-only Electron app with no touch input surface; three controls in a tile's title row need the dense sizing already used by the Controls/Settings rows, and the dashboard grid's own row height is 40px (story `docs/requirements/086-the-dashboard-is-a-grid-i-arrange-myself.md`). |
...
| `/design-tokens` (44px touch-target floor) | The dashboard's Config Profiles tile clickable rows (`src/renderer/src/modules/home/dashboard/ConfigProfilesTile.tsx`, ~28px tall), both tiles' empty-state action buttons (`ConfigProfilesTile.tsx`'s and `PlaytimeTile.tsx`'s, `size="sm"` (28px) Button/IconButtons), the shared frame's error-state retry button (`src/renderer/src/modules/home/components/DashboardTileFrame.tsx`'s `dashboard-tile-frame-retry`, `size="sm"`, shown in every tile's error state) and the tile-body render-error boundary's retry button (`src/renderer/src/modules/home/dashboard/DashboardTile.tsx`'s `dashboard-tile-render-error-retry`, also `size="sm"`) all use sub-floor pointer hit areas. | Same reason as above — a desktop, mouse-and-keyboard-only Electron app with no touch input surface, consistent with the dense sizing already used elsewhere on the dashboard; introduced by story `docs/requirements/087-two-tiles-worth-having-playtime-and-config-profiles.md`. |
