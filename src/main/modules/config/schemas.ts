...
export const configActionSchema = z.object({
  ...
  // both end up as bind keys through the same `normalizeBindKey` path, so a rule
  // that held for one slot and not the other would only mean the two slots
  // disagree about what a key is.
  secondaryKey: z.string().max(20).optional(),
  ...
  // Story 016 (D6): the modifier held while capturing `key`/`secondaryKey`, restricted to the
