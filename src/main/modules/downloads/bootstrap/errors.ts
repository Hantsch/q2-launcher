 ...
 * Not a member of `DOWNLOADS_ERROR_KEYS`, for the same reason `RETAIL_SOURCE_UNVERIFIED` above is
 * not one either: it is decided before the job exists. Reuses the library's own vocabulary rather
 * than inventing a `downloads.*` spelling of it, since it is the same fact `InstallationsService`
 * reports.
 *
 * `INSTALLATION_RUNNING`, the sibling pre-job refusal this file used to export, was removed by
 * story 091 D4: the write guard now defers the write instead of refusing it (see the "write waits,
 * it does not refuse" note on `upgrade-job.ts`).
 */
